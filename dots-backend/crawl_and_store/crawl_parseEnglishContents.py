...
class SentenceTokenizer(object):
    def __init__(self):
        ...
        # 영어 불용어 엑셀 파일을 로드
        self.load_wb = load_workbook(os.path.abspath('workbook/english_stopwords.xlsx'))
    ...
    def url2sentences(self, url):
        ...
        for idx in range(0, len(re_sentences)):
            if len(re_sentences[idx]) <= 10:
                re_sentences[idx - 1] += (' ' + re_sentences[idx])
                re_sentences[idx] = ''
    ...
    def get_nouns(self, sentences):
        ...
        for sentence in sentences:
            ...
            for (taeso, pumsa) in node:
                # 명사만 추출
                if pumsa in ('NN', 'NNS', 'NNP', 'NNPS') and taeso not in self.stopwords and len(taeso) > 1:
                    nouns.append(taeso)
...
class TextRank(object):
    def __init__(self, text):
        ...
        self.nouns = self.sentences
        self.nouns2 = self.sent_tokenize.get_nouns(self.sentences)

        self.graph_matrix = GraphMatrix()
        self.sent_graph = self.graph_matrix.build_sent_graph(self.nouns)
        self.words_graph, self.idx2word = self.graph_matrix.build_words_graph(self.nouns2)
